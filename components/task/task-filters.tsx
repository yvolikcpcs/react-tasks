...
export default function TaskFilters({ tasks }: TaskFiltersProps) {
  ...
  const normalizedTasks = useMemo<NormalizedTask[]>(
    () =>
      tasks.map((task) => {
        const languageName = task.languageName && task.languageName.trim().length > 0
        ...
        return {
          ...
          tags: task.tags?.filter((tag): tag is string => tag.toLowerCase() !== languageName.toLowerCase()) ?? [],
        };
      }),
    [tasks]
  );
  ...
  const languages = useMemo(() => {
    ...
    normalizedTasks.forEach((task) => {
      const raw = task.languageName || UNSPECIFIED_LANGUAGE;
      const key = raw.toLowerCase().trim();
      
      if (!uniqueMap.has(key) || (raw !== key && uniqueMap.get(key) === key)) {
        uniqueMap.set(key, raw);
      }
    });

    const sortedValues = Array.from(uniqueMap.values()).sort((a, b) => 
      a.localeCompare(b)
    );

    return [ALL_FILTER, ...sortedValues];
  }, [normalizedTasks]);

  const languageFilteredTasks = useMemo(
    () =>
      normalizedTasks.filter((task) => {
        if (languageFilter !== ALL_FILTER && task.languageName.toLowerCase() !== languageFilter.toLowerCase()) {
          return false;
        }
        return true;
      }),
